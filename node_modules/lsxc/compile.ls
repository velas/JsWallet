...
compile = (commander, cb)->
    ...
    if commander.bundle?
      ...
      alias = 
        | commander.alias? => "?alias=#{commander.alias}"
        | _ => ""  
      process.title = alias if alias.length > 0
      dynamicCSS =  | commander.putinhtml => """<style>#{bundlec.css}</style>"""
                    | _ => """ <link rel="stylesheet" type="text/css" href="./#{bundle-css}#{alias}">  """
      dynamicHTML = | commander.putinhtml => """<script>#{bundlec.js}</script>"""
                    | _ => """<script type="text/javascript" src="./#{bundle-js}#{alias}"></script>"""
      if commander.html?
          default-template = '''
          <!DOCTYPE html>
          <html lang="en-us">
