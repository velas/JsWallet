require! {
    \prelude-ls : { map, split, filter, find, foldl, drop, take, sum }
...
simulate-validators-stats = (pools, count) ->
    ...
    count-per-index = [0] * pools.length
    for i from 1 to count
        validators = simulate-choose-validators pools-copy
        for validator in validators
            count-per-index[validator.index]++
    return count-per-index
calc-pool-rois = (all-pools, stake) ->
    stat = simulate-validators-stats all-pools, 14600
    for i from 0 to all-pools.length - 1
        all-pools[i].validator-probability = stat[i] / 14600
    return all-pools
query-pools = (store, cb) ->
#    p = fetch ""
