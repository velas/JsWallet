...
calc-reward = (store, web3t)->
    ...
    err, epochs <- web3t.velas.BlockReward.epochsToClaimRewardFrom(store.staking.chosen-pool, staking-address)
    #console.log { epochs }
    return cb err if err?
...
staking-content = (store, web3t)->
    ...
    exit = ->
        staking-address = store.staking.keystore.staking.address
        data = web3t.velas.Staking.withdraw.get-data(staking-address, store.staking.stake-amount-total)
        to = web3t.velas.Staking.address
        amount = 0
        err <- web3t.vlx2.send-transaction { to, data, amount, gas: 1600000, gas-price: 1000000 }
    get-balance = ->
...
fill-pools = ({ store, web3t }, [item, ...rest], cb)->
    ...
    return cb err if err?
    item.stakers = delegators.length
