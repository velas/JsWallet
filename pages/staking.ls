require! {
    ...
    \../history-funcs.ls
    \./icon.ls
    \prelude-ls : { map, split, filter, find }
...
staking-content = (store, web3t)->
    ...
    calc-reward = ->
        ...
        return cb err if err?
        store.staking.reward = reward `div` (10^18)
        store.staking.reward = round5 store.staking.reward
    claim = ->
        staking-address = store.staking.keystore.staking.address
        mining-address =  store.staking.keystore.mining.address
                    ...
                    if store.staking.reward?
                        .pug
                            .pug.balance
                                ...
                                span.pug  VLX
                            button.pug(style=button-primary2-style on-click=claim) #{lang.claim-reward}
                    else
                        button.pug(style=button-primary2-style on-click=calc-reward) #{lang.calculate-reward}
            if false
                .pug.section
                .title.pug
                    h3.pug #{lang.account-index}
                .description.pug
                    .switch-index.pug
